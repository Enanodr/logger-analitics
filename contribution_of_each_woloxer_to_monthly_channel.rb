...
amount_of_slacks_per_channel_per_month = { }
slacks_by_channel.keys.each { |k| amount_of_slacks_per_channel_per_month[k] = {} }

slacks_by_channel.each do |channel, woloxer_data|
  woloxer_data.each do |woloxer, slacks|
    slacks.each do |slack|
      amount_of_slacks_per_channel_per_month[channel][slack['month']] = 0 if amount_of_slacks_per_channel_per_month[channel][slack['month']] == nil
      amount_of_slacks_per_channel_per_month[channel][slack['month']] += 1
    end
  end
end
...
# CSV Headers
csv_array = [['channel_id', 'woloxer_email', 'month', 'contribiution']]
...
amounts_of_messages_per_channel_and_woloxer.each do |channel, woloxer_data|
  woloxer_data.each do |woloxer, monthly_data|
    monthly_data.each do |month, slacks_amount|
      csv_array << [channel, email_by_slack_id[woloxer], month, "#{slacks_amount.fdiv amount_of_slacks_per_channel_per_month[channel][month]}"]
    end
  end
end
# Save CSV File
CSV.open("monthly_contribiution_per_woloxer_per_channel.csv", "w") { |csv| csv_array.each { |line| csv << line } }
